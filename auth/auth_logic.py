...
# '/callback' route needed for the procedure with OAuth2: Retrieving an access token
@auth_logic.route('/callback', methods=['GET'])
def callback():
    # The user has been redirected back from the provider to our registered callback url. With this redirection, in case
    # of success, comes an authorization code included in the redirect URL. We will use that to obtain an access token
    if request.args.get('error') is None:   # Success case
    ...
    else:                                   # Fail case (access denied)
        return jsonify(result='Login fails: fed4fire access denied'), 400
